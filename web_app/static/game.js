let ws, myName = null, isSpectator = false;
...
function connect(name, spectator) {
  ...
  ws.onmessage = (event) => {
    ...
    if (msg.type === 'joined') {
      myName = msg.name;
      whoLine.innerHTML = isSpectator ? 'Spectator view' : 'Playing as <b>' + myName + '</b>';
      joinCard.style.display = 'none';
      stage.classList.add('active');
      if (isSpectator) { dialPanel.style.display = 'none'; resetBtn.style.display = 'inline-block'; }
...
function renderBoard(rows) {
  if (!rows.length) { board.innerHTML = '<div class="empty-state">Waiting for players…</div>'; return; }
